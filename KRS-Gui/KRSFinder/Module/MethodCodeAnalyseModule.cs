            ...
            while (tempRegisterMap.Count != 0)
            {
                ...
                foreach (string k in temp.Keys)
                {
                    ...
                    foreach (TempRegisterMap registerMap in tempList.Keys)
                    {
                        ...
                        analysedMap = tempRegisterMap[k].GetValueOrDefault(registerMap, new List<string>());
                        //}
                        ...
                        case OpCode.TYPE_MUST:
                            {
                                ...
                                else if (opCode == OpCode.IF_OPC_MUST)
                                {
                                    ...
                                    if (rewalk)
                                    {
                                        register.isLoop = true;
                                    }
                                    ifhandler.setTargetBlockTempRegister(tempRegisterMap, register, analysedMap);
